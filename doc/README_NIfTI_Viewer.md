...
| **图像缩放** | **Ctrl** (或 Command) + **鼠标滚轮** | 向上放大，向下缩小 (非编辑工具调整时) |
| **调整工具** | **Ctrl** (或 Command) + **鼠标滚轮** | **画笔/橡皮**: 调整笔刷大小 <br> **魔棒**: 调整容差阈值 |
...
*   **工具**:
    *   **画笔/橡皮**: 大小可调节 (1-20)。支持使用 **Ctrl+滚轮** 快速调整大小。快速绘制时支持自动插值，保证线条平滑连续。
    *   **魔棒**: 点击区域根据灰度阈值自动选取连通区域。支持使用 **Ctrl+滚轮** 快速调整阈值 (0-180)。
    *   **填充**: 点击某个标签区域，自动将所有**连通且标签值相同**的区域填充为当前选定的标签值。
*   **导出 Label**:
    *   点击“导出 Label”按钮。
